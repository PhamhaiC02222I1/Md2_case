...
public class ProductServiceIMPL implements IProductServiceIMPL {
    ...
    public static List<Product> productList = new ConfigReadAndWriteFile<Product>().readFromFile(PATH);

    @Override
    public List<Product> findAll() {
        new ConfigReadAndWriteFile<Product>().writeToFile(PATH, productList);
        return productList;
    }
    ...
    @Override
    public Product findByCatagoryName(String type) {
        for (int i = 0; i < productList.size(); i++) {
            if (type.equals(productList.get(i).getType())) {
    ...
    public Product findProductByPrice2() {
        for (int i = 0; i < productList.size(); i++) {
            if (productList.get(i).getPrice() < 10000&&productList.get(i).getPrice()>5000) {
                System.out.println(productList.get(i));
